        ...
        <div class="card-body">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-3">
                    @if ($upload->status === 'completed')
                        <span class="badge badge-success" style="font-size:13px;padding:6px 14px"><span
                                class="badge-dot"></span> Completed</span>
                    @elseif($upload->status === 'processing')
                        <span class="badge badge-info" style="font-size:13px;padding:6px 14px"><span
                                class="badge-dot"></span> Processing</span>
                    @elseif($upload->status === 'failed')
                        <span class="badge badge-error" style="font-size:13px;padding:6px 14px"><span
                                class="badge-dot"></span> Failed</span>
                    @else
                        <span class="badge badge-pending" style="font-size:13px;padding:6px 14px"><span
            ...
            <div style="margin-bottom:16px">
                ...
                <div class="progress-container" style="height:10px">
                    <div class="progress-bar {{ $upload->status === 'processing' ? 'animated' : '' }}" id="progress-bar"
            ...
            <div class="stats-grid" style="margin-bottom:0">
                <div class="stat-card green" style="padding:16px">
                    <div class="stat-value" style="font-size:22px" id="stat-success">
                        {{ number_format($upload->success_rows) }}</div>
                    <div class="stat-label">Berhasil</div>
                </div>
                <div class="stat-card orange" style="padding:16px">
                    <div class="stat-value" style="font-size:22px" id="stat-error">{{ number_format($upload->error_rows) }}
                    </div>
                    <div class="stat-label">Error</div>
                </div>
                <div class="stat-card blue" style="padding:16px">
                    <div class="stat-value" style="font-size:22px" id="stat-warning">
                        {{ number_format($upload->warning_rows) }}</div>
                    <div class="stat-label">Warning</div>
                </div>
                <div class="stat-card purple" style="padding:16px">
                    <div class="stat-value" style="font-size:22px" id="stat-total">{{ number_format($upload->total_rows) }}
                    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            @if ($errorLogs->count() > 0)
                <h4 style="font-size:14px;color:var(--error);margin-bottom:12px"><i class="ri-error-warning-line"></i>
                    Errors</h4>
                @foreach ($errorLogs as $log)
                    <div class="log-entry error">
                        ...
                        <div>
                            ...
                            @if ($log->row_number)
                                Baris {{ $log->row_number }}:
                            @endif
                            {{ $log->message }}
                            <div class="text-sm text-muted" style="margin-top:4px">{{ $log->created_at->format('H:i:s') }}
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            @if ($warningLogs->count() > 0)
                <h4 style="font-size:14px;color:var(--warning);margin:20px 0 12px"><i class="ri-alert-line"></i> Warnings
                </h4>
            ...
            @if ($infoLogs->count() > 0)
                <h4 style="font-size:14px;color:var(--info);margin:20px 0 12px"><i class="ri-information-line"></i> Info
                </h4>
                ...
                let pollInterval = setInterval(function() {
                    fetch(`/api/upload/${uploadId}/progress`)
                        ...
                        .then(data => {
                            ...
                            document.getElementById('stat-success').textContent = data.success_rows.toLocaleString(
                                'id-ID');
                            document.getElementById('stat-error').textContent = data.error_rows.toLocaleString(
                                'id-ID');
                            document.getElementById('stat-warning').textContent = data.warning_rows.toLocaleString(
                                'id-ID');
                            ...
                            if (data.status === 'completed' || data.status === 'failed') {
                                ...
                                showToast(data.status === 'completed' ? 'Import selesai!' : 'Import gagal!', data
