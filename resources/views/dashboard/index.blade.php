...
<div class="grid-2">
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="ri-store-2-line" style="margin-right:8px;color:var(--accent-hover)"></i>Penjualan per Kanal</h3>
        </div>
        <div class="card-body">
            @if($salesByKanal->count() > 0)
                ...
                <div class="chart-bar" style="margin-bottom: 40px;">
                    @foreach($salesByKanal as $kanal)
                        <div class="chart-bar-item" style="height: {{ max(20, ($kanal->total_revenue / $maxRevenue) * 100) }}%">
                            <span class="chart-bar-value">Rp {{ number_format($kanal->total_revenue / 1000, 0) }}K</span>
                        ...
                        <tbody>
                            @foreach($salesByKanal as $kanal)
                                <tr>
                                    ...
                                    <td class="text-right">Rp {{ number_format($kanal->total_revenue, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach($topProducts as $idx => $prod)
                                <tr>
                                    <td>
                                        @if($idx === 0) <span style="color:gold">🥇</span>
                                        @elseif($idx === 1) <span style="color:silver">🥈</span>
                                        @elseif($idx === 2) <span style="color:#cd7f32">🥉</span>
                                        @else {{ $idx + 1 }}
                                        @endif
                                    </td>
                                    <td>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($recentUploads as $up)
                            <tr>
                                <td><a href="{{ route('upload.show', $up->id) }}" style="color:var(--accent-hover);text-decoration:none">{{ $up->batch_code }}</a></td>
                                <td>
